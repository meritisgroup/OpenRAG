...
def get_rag_agent(rag_name, config_server, models_infos, databases_name=[""]):
    ...
    if rag_name == "naive":
        ...
        agent = QueryReformulationRag(models_infos=models_infos,
                            config_server=config_server,
                            dbs_name=names,
                            data_folders_name=databases_name)
    elif rag_name == "graph":
...
def get_custom_rag_agent(base_rag_name, config_server, models_infos, databases_name=[""]):
    names = [config_server["name"] + "_" + "_" + re.sub(r"[^a-zA-Z0-9]", "_", config_server["embedding_model"]) + "_" + "_" + database_name for database_name in databases_name]
    ...
    if base_rag_name == "naive":
        ...
        agent = SemanticChunkingRagAgent(
            config_server=config_server, models_infos=models_infos, dbs_name=names, data_folders_name=databases_name
        )
    return agent
