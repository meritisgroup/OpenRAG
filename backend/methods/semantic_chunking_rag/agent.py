...
class SemanticChunkingRagAgent(RagAgent):
    ...
    def __init__(
        ...
        self.db = get_database(db_name=self.db_name, 
    ...
    def indexation_phase(
        ...
        if not overlap:
            index.run_pipeline(batch=True, chunk_size=chunk_size, overlap_size=0)
        else:
            index.run_pipeline(batch=True, chunk_size=chunk_size)
