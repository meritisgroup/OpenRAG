...
from ..advanced_rag.reranker import Reranker
...
class BaseAgent(ABC):
    ...
    def process(self, input_data: Any, **kwargs) -> AgentResponse:
        """Traite une entrée et retourne une réponse"""
        pass


class RouterAgent(BaseAgent):
    """Agent de routage qui analyse et planifie la requête"""
    ...
    def process(self, input_data: str, **kwargs) -> AgentResponse:
        """Analyse la requête et crée un plan d'exécution"""
        ...
        response = self.agent.predict_json(prompt=routing_prompt,
        ...
        try:
            return AgentResponse(content=response,
                                 ...
                                 metadata=response.dict()), 0, 0
        
        except Exception as e:
            return AgentResponse(content=QueryPlan(
                            ...
                            confidence=0.5,
                            reasoning=f"Erreur d'analyse, utilisation du plan par défaut: {str(e)}",
                            metadata={"error": str(e)}
                        ), 0, 0
        

class RetrieverAgent(BaseAgent):
    """Agent de récupération avec stratégies multiples"""
    ...
    def process(self, input_data: Dict, **kwargs) -> AgentResponse:
        """Récupère les chunks pertinents selon la stratégie"""
        ...
        if self.reranker and len(chunk_list) > 0:
            reranked_chunks, metadata, nb_tokens = self.reranker.rerank(query=query,
                                                                        chunk_list=chunk_list,
                                                                        max_contexts=nb_chunks)
        ...
        return AgentResponse(content=reranked_chunks,
                            confidence=confidence,
                            reasoning=f"Récupération de {len(reranked_chunks)} chunks avec stratégie {strategy}",
                            metadata={"total_chunks": len(chunk_list),
...
class ReasonerAgent(BaseAgent):
    ...
    def process(self, input_data: Dict, **kwargs) -> AgentResponse:
        """Effectue un raisonnement sur les chunks récupérés"""
        query = input_data["query"]
        ...
        response = self.agent.predict(prompt=reasoning_prompt,
                                      system_prompt=system_prompt,
                                      temperature=0.2,
                                      model= self.llm_model)
        ...
        return AgentResponse(
            ...
            metadata={"nb_chunks_analyzed": len(chunks)}
        ), input_tokens, output_tokens


class SynthesizerAgent(BaseAgent):
    """Agent de synthèse pour combiner plusieurs sources"""
    ...
    def process(self, input_data: Dict, **kwargs) -> AgentResponse:
        """Synthétise les informations de plusieurs sources"""
        query = input_data["query"]
        sub_responses = input_data["sub_responses"]
        synthesis_prompt = self.prompts["synthesis"]["QUERY_TEMPLATE"].format(query=query,
    ...
    def _format_sub_responses(self, sub_responses: List[Dict]) -> str:
        """Formate les réponses partielles"""
        formatted = []
        for i, resp in enumerate(sub_responses, 1):
            formatted.append(
                f"\n--- Réponse {i} (confiance: {resp.get('confidence', 'N/A')}) ---\n"
                f"{resp.get('content', 'N/A')}\n"
            )
        return "\n".join(formatted)


class EvaluatorAgent(BaseAgent):
    """Agent d'évaluation de la qualité des réponses"""
    ...
    def process(self, input_data: Dict, **kwargs) -> AgentResponse:
        ...
        response = self.agent.predict(prompt=eval_prompt,
                                      system_prompt=system_prompt,
                                      temperature=0.1,
                                      model= self.llm_model)
        ...
        try:
            ...
            return AgentResponse(
                content={"score_global": 0.5},
                confidence=0.5,
                reasoning="Évaluation par défaut",
                metadata={"error": "Parsing error"}
            ), input_tokens, output_tokens