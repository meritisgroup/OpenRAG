...
class Processor_chunks:
    ...
    def process_chunk(self, chunks, doc_content, model):
        ...
        if "Global_sum_up" in self.type_processor_chunks:
            data_temp = self.run_global_sum_up(chunks=data["chunks"],
        ...
        if "Extractor_metadata" in self.type_processor_chunks:
            data_temp = self.run_extractor_metadata(chunks=data["chunks"],
                                                    doc_content=doc_content,
                                                    model=model,
        ...
        if "Contextual" in self.type_processor_chunks:
            data_temp = self.run_contextual(chunks=data["chunks"],
                                                doc_content=doc_content,
                                                model=model,
                                                first_process=first_process)
            data["chunks"] = data_temp["chunks"]
    ...
    def run_global_sum_up(self, chunks, doc_content, model):
        data = run_global_sum_up(chunks=chunks,
                                 doc_content=doc_content,
                                 model=model,
                                 agent=self.agent,
    ...
    def run_extractor_metadata(self, chunks, doc_content, model, first_process):
        data = run_extract_metadata(chunks=chunks,
                                    doc_content=doc_content,
                                    model=model,
                                    agent=self.agent,
                                    language=self.language,
...
def run_extract_metadata(chunks, agent, model, doc_content, language, first_process):
    ...
    prompt_metadata = prompts[language]["extract_metadata"]["QUERY_TEMPLATE"].replace("{WHOLE_DOCUMENT}",
                                                                                      chunks_resume)

    context = agent.predict(prompt=prompt_metadata,
...
def run_global_sum_up(chunks, agent, model, doc_content, language):
    ...
    system_prompts = [prompts[language]["generate_global_sum"]["SYSTEM_PROMPT"] for i in range(len(chunks))]
    ...
    contexts = agent.multiple_predict(prompts=prompts_resume,
    ...
    if len(chunks_resume)>1:
        ...
        contexts = agent.predict(prompt=prompt,
                                system_prompt=system_prompt,
                                model=model)
        nb_output_tokens += np.sum(contexts["nb_output_tokens"])
        nb_input_tokens += np.sum(contexts["nb_input_tokens"])
