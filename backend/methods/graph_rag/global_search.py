...
class GlobalSearch(Search):
    ...
    def get_context(self, query: str) -> str:
        ...
        useful_communities = {}
        useful_entities = {}
        top_k_communities = self.vb.k_search(
            queries=query, collection_name=self.db.name[:-3] + "_global_search", k=self.pre_filter
        )
        ...
        for title in top_k_communities[0]:
            try:
                ...
                print("Error while getting usefull communities : ", e)
    ...
    def _evaluate_communities(self, query, context):
        ...
        answer = self.agent.predict(system_prompt=system_prompt, prompt=prompt)
        return answer
