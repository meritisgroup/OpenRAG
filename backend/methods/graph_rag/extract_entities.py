...
def save_chunks_to_jsonl(doc_id, chunks: List[Chunk], jsonl_path: str = "chunks_output.jsonl") :
...
class DocumentText:
    def __init__(self, doc_index ,path: str, config_server : dict, splitter: Splitter = TextSplitter()):
        ...
        try:
            if self.data_preprocessing=="md_with_images":
                self.content = MarkdownOpener(config_server=self.config_server).open_doc(path_file=path)
            elif self.data_preprocessing=="md_without_images":
                self.content = MarkdownOpener(config_server=self.config_server,image_description = False).open_doc(path_file=path)
            else : 
        ...
        if self.data_preprocessing=="md_with_images" or self.data_preprocessing=="md_without_images":
        ...
        if self.data_preprocessing=="md_with_images" or self.data_preprocessing=="md_without_images":
            ...
            for section in md_sections:
                ...
                for text in texts:
                    ...
                    results.append(Chunk(text=final_text, document=self.name_with_extension, position_in_doc=chunk_position, id=chunk_id))
                    chunk_position += 1
            ...
            for k, text in enumerate(texts):
                results.append(
                    Chunk(text=text, document=self.name_with_extension, position_in_doc=k + 1, id=make_chunk_id())
                )

        #save chunks  
        save_chunks_to_jsonl(self.doc_index,
        results
)
        """
    ...
    def convert_in_base(self) -> Document:
        return Document(name=self.name_with_extension, embedding_tokens=0, input_tokens=0, output_tokens=0)
