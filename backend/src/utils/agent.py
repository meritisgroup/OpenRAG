...
def get_Agent(config_server: dict, models_infos: dict):
    ...
    max_attempts = config_server.get('max_attempts', 5)
    max_workers = config_server.get('max_workers', 10)
    agent = Agent_openai(models_infos=models_infos, language=language, max_attempts=max_attempts, max_workers=max_workers)
    return agent
...
class Agent_openai(Agent):
    ...
    def __init__(self, models_infos, url: str=None, language='EN', max_attempts=5, max_workers=10):
    ...
    def predict_images(self, prompts: list[str], model: str, images: list[np.ndarray], json_format: BaseModel=None, temperature: float=None, max_workers: int=10):
    ...
    def predict_image(self, prompt: str, model: str, image: np.ndarray, json_format: BaseModel=None, temperature: float=None):
    ...
    def predict(self, prompt: str, system_prompt: str, model: str, temperature: float=0, options_generation=None) -> str:
    ...
    def predict_json(self, prompt: str, system_prompt: str, model: str, json_format, temperature: float=0, options_generation=None):
        return predict_json(system_prompt, prompt, model, self.clients[model], json_format, temperature=temperature)
    ...
    def reranking(self, query: str, chunk_list: list[Chunk], model, max_workers: int=10):
        ...
        if self.models_infos[model]['type'] == 'reranker':
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for (i, chunk) in enumerate(chunk_list):
                    ...
                    future = executor.submit(rerank, system_prompt, prompt, model, self.clients[model], temperature=None)
