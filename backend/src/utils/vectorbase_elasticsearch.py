...
def get_mapping_vb_embedding(class_: Type[DeclarativeMeta], vector_dims: int) -> dict:
    ...
    for column in class_.__table__.columns:
        if isinstance(column.type, Integer):
            ...
            es_properties[column.name] = {'type': 'text'}
    es_properties['vector'] = {'type': 'dense_vector', 'dims': vector_dims, 'index': True, 'similarity': 'cosine'}
    return {'mappings': {'properties': es_properties}}

def get_mapping_to_bm25(class_: Type[DeclarativeMeta], k1: float=1.2, b: float=0.75) -> dict:
    es_properties = {}
    for column in class_.__table__.columns:
        if isinstance(column.type, Integer):
            es_properties[column.name] = {'type': 'integer'}
    ...
    mapping = {'settings': {'similarity': {'default': {'type': 'BM25', 'k1': k1, 'b': b}}}, 'mappings': {'properties': es_properties}}
    return mapping
...
class VectorBase_embeddings_elasticsearch(VectorBase):
    ...
    def create_collection(self, name=None, add_fields=[]) -> None:
        ...
        if len(add_fields) > 0:
            for field in add_fields:
                ...
                mapping['mappings']['properties'][field['field_name']] = {'type': field['data']['datatype']}
    ...
    def add_str_batch_elements(self, chunks: list[Chunk], display_message: bool=True, collection_name=None) -> None:
        if collection_name is None:
            collection_name = self.vb_name
        ...
        if chunks:
            ...
            for (k, chunk) in enumerate(chunks):
                source = chunk_to_dict_vb_embedding(chunk=chunk, embedding=embeddings['embeddings'][k])
                temp = {'_index': collection_name, '_source': source}
                data.append(temp)
    ...
    def k_search(self, queries: Union[str, list[str]], k: int, output_fields: list[str]=['text'], filters: dict=None, collection_name=None, type_output=Chunk):
        ...
        for i in range(len(data['embeddings'])):
            body = {'size': k, 'knn': {'field': 'vector', 'query_vector': data['embeddings'][i], 'k': k, 'num_candidates': num_candidates}}
            try:
...
class VectorBase_BM25_elasticsearch(VectorBase):
    ...
    def k_search(self, queries: Union[str, list[str]], k: int, output_fields: list[str]=['text'], filters: dict=None, collection_name=None, type_output=Chunk):
        ...
        for i in range(len(queries)):
            response = self.client.search(index=collection_name, body={'size': k, 'query': {'match': {'text': queries[i]}}})
            res.append(response)
        results = []
        for l in range(len(res)):
            result = []
...
class VectorBase_hybrid_elasticsearch(VectorBase_embeddings_elasticsearch):
    ...
    def k_search(self, queries: Union[str, list[str]], k: int, output_fields: list[str]=['text'], filters: dict=None, collection_name=None, type_output=Chunk) -> list[Chunk]:
        ...
        for i in range(len(queries)):
            response = self.client.search(index=collection_name, body={'size': k, 'query': {'script_score': {'query': {'bool': {'should': [{'match': {'text': queries[i]}}, {'knn': {'field': 'vector', 'query_vector': data['embeddings'][i], 'k': k, 'num_candidates': num_candidates}}]}}, 'script': {'source': "0.5 * _score + 0.5 * cosineSimilarity(params.query_vector, 'vector')", 'params': {'query_vector': data['embeddings'][i]}}}}})
            res.append(response)
        results = []
        for l in range(len(res)):
            result = []
...
class VectorBaseVlm_elasticsearch:
    ...
    def __init__(self, vb_name: str, url_elasticsearch: str, embedding_model, agent, auth):
        self.vb_name = vb_name
        self.auth = auth
        self.agent = agent
        self.url_elasticsearch = url_elasticsearch
        self.embedding_model = embedding_model
        self.dimension = len(self.agent.embeddings_vlm(queries='test', model=embedding_model, mode='vlm')['embeddings'][0])
    ...
    def get_current_doc_id(self, collection_name=None):
        ...
        if not self.check_collection_exist(collection_name=collection_name):
            return 0
        response = self.client.search(index=collection_name, body={'size': 0, '_source': False, 'aggs': {'max_doc_id': {'max': {'field': 'doc_id'}}}})
        return response['aggregations']['max_doc_id']['value']

    def create_collection(self, name=None) -> None:
        if name is None:
            name = self.vb_name
        mapping = {'mappings': {'properties': {'vector': {'type': 'dense_vector', 'dims': self.dimension, 'index': True, 'similarity': 'cosine'}, 'id': {'type': 'long'}, 'doc_id': {'type': 'long'}, 'path': {'type': 'text'}}}}
        if not self.check_collection_exist(collection_name=name):
            self.client.indices.create(index=name, body=mapping)
        else:
            print(f'The collection "{name}" already exists')
    ...
    def add_str_elements(self, elements: list[str], doc_id: int, metadata: dict=None, display_message: bool=True, collection_name=None) -> None:
        if collection_name is None:
            collection_name = self.vb_name
        if not self.check_collection_exist(collection_name=collection_name):
            self.create_collection(name=collection_name)
        filtered_elements = [element for element in elements if not self.check_element_exist(element=element, collection_name=collection_name)]
        data = []
    ...
    def k_search(self, queries: Union[str, list[str]], k: int, output_fields: list[str]=['text'], filters: dict=None, collection_name=None):
        ...
        if type(queries) is str:
            queries = [queries]
        data = self.agent.embeddings_vlm(queries=queries, model=self.embedding_model, mode='vlm')
        res = []
        num_candidates = max(500, k * 5)
        for i in range(len(data['embeddings'])):
            response = self.client.search(index=collection_name, body={'size': k, 'knn': {'field': 'vector', 'query_vector': data['embeddings'][i], 'k': k, 'num_candidates': num_candidates}})
            res.append(response)
        results = []
        for l in range(len(res)):
            result = []
