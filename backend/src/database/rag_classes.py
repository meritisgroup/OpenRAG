...
class Chunk(Base):
    ...
    def to_dict(self) -> dict:
        return {'id': self.id, 'position_in_doc': self.position_in_doc, 'document': self.document, 'text': self.text, 'rerank_score': self.rerank_score}
...
class Chunk_query(Base):
    ...
    def from_chunk(cls, chunk: Chunk, query: str):
        return cls(id=chunk.id, position_in_doc=chunk.position_in_doc, document=chunk.document, text=query, text_doc=chunk.text, rerank_score=chunk.rerank_score)
    ...
    def to_dict(self) -> dict:
        return {'id': self.id, 'position_in_doc': self.position_in_doc, 'document': self.document, 'text': self.text, 'text_doc': self.text_doc, 'rerank_score': self.rerank_score}
...
class DocumentText:
    ...
    def chunks(self, chunk_size: int=1024, chunk_overlap: bool=True) -> list[Chunk]:
        ...
        for (k, chunk) in enumerate(chunks):
            results.append(Chunk(text=chunk, document=self.name_with_extension, position_in_doc=k + 1))
        return results

    def convert_in_base(self) -> Document:
        return Document(name=self.name_with_extension, path=self.path, embedding_tokens=self.embedding_tokens, input_tokens=self.input_tokens, output_tokens=self.output_tokens)
