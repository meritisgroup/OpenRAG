from sqlalchemy import inspect, create_engine, delete, MetaData, Column, String, Table, JSON
...
class Merger_Database_Vectorbase:
    ...
    def add_database(self, db_name, data_folder_name, embedding_model) -> None:
        ...
        data_path = os.path.join(self.config_server['storage_data_path'], data_folder_name)
        ...
        if not self.auto_sharding:
            self.add_vectorbase(vb_name=db_name, embedding_model=embedding_model)
    ...
    def create_merged_entities_table(self, db_name: str=None, overall=True, path: str=None):
        if db_name is None and path is not None:
            db_name = self.find_db_name_path(path)
        if db_name is not None:
            self.databases[db_name]['database'].create_merged_entities_table(agent=self.agent, overall=overall)
        else:
            for db_name in self.databases.keys():
                self.databases[db_name]['database'].create_merged_entities_table(agent=self.agent, overall=overall)
    ...
    def query_filter(self, table_class: Base, filter, db_name: str=None, path_docs: str=None):
        if db_name is None and path_docs is not None:
            db_name = self.find_db_name_path(path_docs)
        if db_name is not None:
    ...
    def create_collection(self, vb_name: str=None, name: str=None, add_fields=[]) -> None:
        if vb_name is not None:
            if name is not None:
                ...
                name_create = vb_name
            self.vectorbases[vb_name]['vectorbase'].create_collection(name=name_create, add_fields=add_fields)
        else:
            for vb_name in self.vectorbases.keys():
                if name is not None:
                    name_create = vb_name + '_' + name
    ...
    def add_str_elements(self, chunks=list[Chunk], path_docs: list[str]=None, display_message: bool=True, collection_name=None, vb_name: str=None) -> None:
        if vb_name is None:
    ...
    def add_str_batch_elements(self, chunks: list[Chunk], path_docs: list[str]=None, display_message: bool=True, collection_name=None, vb_name: str=None) -> None:
        if vb_name is None:
            vb_name = self.find_vb_name(path_docs)
        elif collection_name is not None and vb_name is not None:
            collection_name = vb_name + '_' + collection_name
    ...
    def k_search(self, queries: Union[str, list[str]], k: int, output_fields: list[str]=['text'], filters: dict=None, collection_name=None, vb_name: str=None, type_output=Chunk):
        if vb_name is not None:
            if collection_name is not None:
                collection_name = vb_name + '_' + collection_name
            return self.vectorbases[vb_name]['vectorbase'].k_search(queries=queries, k=k, output_fields=output_fields, filters=filters, collection_name=collection_name, type_output=type_output)
        else:
            ...
            for i in range(len(self.vectorbases)):
                ...
                result = self.vectorbases[vb_name]['vectorbase'].k_search(queries=queries, k=k_per_db_list[i], output_fields=output_fields, collection_name=collection_name_search, type_output=type_output)
                if len(chunks) == 0:
                    chunks = result
                else:
...
class DataBase:
    ...
    def create_merged_entities_table(self, agent: Agent, overall=True) -> None:
        ...
        if overall:
            ...
            already_in_entities = [res[0] for res in self.session.query(MergeEntityOverall.name).all()]
            ...
            with tqdm(entities_to_process) as progress_bar:
                for (k, entity_name) in enumerate(progress_bar):
                    ...
                    entities = self.session.query(Entity).filter(Entity.name == entity_name).all()
            ...
            with tqdm(entity_names) as progress_bar:
                for (k, entity_name) in enumerate(progress_bar):
                    ...
                    for doc_name in doc_names:
                        already_in_entities = [res[0] for res in self.session.query(MergeEntityDocument.name).filter(MergeEntityDocument.doc_name == doc_name).all()]
                        if entity_name not in already_in_entities:
                            filtered_entities = [entity for entity in entities if entity.doc_name == doc_name]
                            descriptions = [entity.description for entity in filtered_entities]
                            chunk_ids = [entity.chunk_id for entity in filtered_entities]
                            kind = filtered_entities[0].kind
                            description = DescriptionClean(descriptions=descriptions).clean_description(agent=agent)
                            new_entity = MergeEntityDocument(name=entity_name, kind=kind, description=description, chunk_ids=chunk_ids, doc_name=doc_name, degree=len(filtered_entities))
                            self.session.add(new_entity)
                            self.session.commit()
                    if k == len(progress_bar) - 1:
                        progress_bar.set_description('Merging multiples occurences - ✅')
...
class ContextDatabase:
    ...
    def __init__(self, db_name: str='context_rags.db', db_folder_path: str='./storage'):
    ...
    def create_rag_table(self, rag_name: str):
        rag_table = Table(rag_name, self.metadata, Column('query', String, primary_key=True), Column('context', JSON), extend_existing=True)
