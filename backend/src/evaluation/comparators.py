...
class ArenaBattle:
    ...
    def __init__(self, dataframe: pd.DataFrame, agent: Agent, model: str, eval_number=1, max_attempts=5, batch_size=10):
        ...
        self.rag_list = [col for col in dataframe.columns if col != 'QUERIES' and col != 'GROUND_TRUTH']
        self.agent = agent
        self.model = model
    ...
    def process_round_scores(self, first_rag: str, second_rag: str):
        ...
        for eval_idx in range(self.eval_number):
            dict_scores = self.metric_evaluator.run_evaluation_pipeline(self.queries, self.ground_truths, first_answers, second_answers)
    ...
    def run_battles_scores(self, log_file):
        ...
        all_scores_dict = {}
        total_comparisons = num_rags * (num_rags - 1) // 2
        progress_bar = ProgressBar(total=total_comparisons, desc='Arena Battles Progress')
        k = 0
        ...
        for i in range(num_rags):
            for j in range(i + 1, num_rags):
                ...
                all_scores_dict[f'{first_rag}_v_{second_rag}'] = {metric: [0, 0] for metric in self.metrics}
                ...
                for metric in self.metrics.keys():
                    ...
                    all_scores_dict[f'{first_rag}_v_{second_rag}'][metric] = [first_result, second_result]
                progress_bar.update(k, text=f'Arena Battles Progress ({k + 1} / {int(num_rags * int(num_rags - 1) / 2)})')
                k += 1
...
class GroundTruthComparator:
    ...
    def process_evaluation(self, rag_answers: list[str], rag_name: str):
        ...
        for eval_idx in range(self.eval_number):
            dict_scores = self.comparator.run_evaluation_pipeline(self.queries, self.ground_truth, rag_answers, rag_name=rag_name)
            for metric in self.metrics.keys():
                try:
    ...
    def run_evaluations(self, log_file):
        ...
        all_scores_dict = {rag: {metric: 0 for metric in self.metrics} for rag in self.rag_list}
        ...
        for (i, rag) in enumerate(self.rag_list):
            progress_bar.update(i - 1, text=f'Processing ground truth comparisons for {rag} rag ({i + 1} / {n})')
...
class ContextRelevanceComparator:
    ...
    def run_evaluations(self, log_file):
        ...
        progress_bar = ProgressBar(total=num_rags, desc='Context Relevance comparison progress')
        n = len(self.rag_list)
        for (i, rag) in enumerate(self.rag_list):
            progress_bar.update(i - 1, text=f'Processing context relevance evaluations for {rag} rag ({i + 1} / {n})')
...
class ContextFaithfulnessComparator:
    ...
    def __init__(self, dataframe: pd.DataFrame, agent: Agent, model: str, eval_number=1, max_attempts=5, batch_size=10):
        self.agent = agent
        self.model = model
        self.evaluator = ContextFaithfulnessEvaluator(agent=self.agent, model=model, max_attempts=max_attempts, batch_size=batch_size)
        self.eval_number = eval_number
        self.dataframe = dataframe
    ...
    def run_evaluations(self, log_file):
        ...
        num_rags = len(self.rag_list)
        all_scores = {f'{rag}': 0.0 for rag in self.rag_list}
        progress_bar = ProgressBar(total=num_rags, desc='Context faithfulness comparison progress')
        n = len(self.rag_list)
        for (i, rag) in enumerate(self.rag_list):
            progress_bar.update(i - 1, text=f'Processing context faithfulness evaluations for {rag} rag ({i + 1} / {n})')
...
class nDCGComparator:
    ...
    def __init__(self, dataframe: pd.DataFrame, agent: Agent, model: str, eval_number=1, max_attempts=5, batch_size=10):
        self.agent = agent
        self.model = model
        self.evaluator = nDCGEvaluator(agent=self.agent, model=model, max_attempts=max_attempts, batch_size=batch_size)
        self.eval_number = eval_number
        self.dataframe = dataframe
        self.queries = dataframe['QUERIES'].tolist()
        self.rag_list = [col for col in dataframe.columns if col != 'QUERIES' and col != 'GROUND_TRUTH']
    ...
    def run_evaluations(self, log_file):
        ...
        num_rags = len(self.rag_list)
        all_scores = {f'{rag}': 0.0 for rag in self.rag_list}
        progress_bar = ProgressBar(total=num_rags, desc='nDCG comparison progress')
        n = len(self.rag_list)
        for (i, rag) in enumerate(self.rag_list):
            progress_bar.update(i - 1, text=f'Processing nDCG evaluations for {rag} rag ({i + 1} / {n})')
            contexts = [row['CONTEXT'] for row in self.dataframe[rag]]
            if all((context == [] for context in contexts)):
                score = 0
            else:
                score = self.process_evaluation(self.ground_truth, contexts)
