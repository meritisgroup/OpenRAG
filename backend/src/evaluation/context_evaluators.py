...
class ContextRelevanceEvaluator(Evaluator):
    ...
    def _get_prompts(self, queries: list[str], model_contexts: list[str]) -> tuple[list[str], str]:
        system_prompt = self.system_prompt
        prompts = [self.prompt_template.replace('{query}', str(query)).replace('{model_context}', str(model_context)) for (query, model_context) in zip(queries, model_contexts)]
        return (prompts, system_prompt)

    def _clean_output(self, output: str) -> str:
        insufficient_answer = re.compile('Insufficient Information', re.IGNORECASE)
        empty_output = ''
        if insufficient_answer.search(output):
            return empty_output
        elif re.compile('Informations insuffisantes', re.IGNORECASE).search(output):
            return empty_output
        else:
            return output.strip()

    def _get_evaluation(self, queries: list[str], model_contexts: list[str]) -> list[float | None]:
        (prompts, system_prompt) = self._get_prompts(queries, model_contexts)
        cleaned_outputs = process_prompts(prompts=prompts, system_prompts=[system_prompt], max_retry=self.max_attempts, agent=self.agent, model=self.model, clean_output=self._clean_output)
        context_relevance_evaluations = []
        for (clean_output, model_context) in zip(cleaned_outputs, model_contexts):
            if len(clean_output) > 0:
                context_relevance_evaluations.append(len(clean_output.split('.')) * 100 / len(model_context.split('.')))
            else:
                context_relevance_evaluations.append(0)
        return context_relevance_evaluations

    def run_evaluation_pipeline(self, queries: list[str], model_contexts: list[str]) -> float:
...
class ContextFaithfulnessEvaluator(Evaluator):
    ...
    def __init__(self, agent: Agent, model: str, max_attempts: int=5, batch_size: int=10) -> None:
        ...
        self.system_prompt_statements: str = PROMPTS[self.language]['get_statements']['SYSTEM_PROMPT']
        self.prompt_template_statements: str = PROMPTS[self.language]['get_statements']['QUERY_TEMPLATE']
        ...
        self.system_prompt_faithfulness: str = PROMPTS[self.language]['rate_context_faithfulness']['SYSTEM_PROMPT']
        self.prompt_template_faithfulness: str = PROMPTS[self.language]['rate_context_faithfulness']['QUERY_TEMPLATE']
    ...
    def _get_prompts_statements(self, queries: list[str], answers: list[str]) -> tuple[list[str], str]:
        system_prompt = self.system_prompt_statements
        prompts = [self.prompt_template_statements.replace('{query}', str(query)).replace('{answer}', str(answer)) for (query, answer) in zip(queries, answers)]
        return (prompts, system_prompt)
    ...
    def _get_statements(self, queries: list[str], answers: list[str]):
        ...
        statements: list[str] = process_prompts(prompts=prompts, system_prompts=[system_prompt], max_retry=self.max_attempts, agent=self.agent, model=self.model, clean_output=self._clean_output_statements)
        return statements

    def _get_prompts_faithfulness(self, statements: list[str], model_context: str) -> tuple[list[str], str]:
        system_prompt = self.system_prompt_faithfulness
        prompts = [self.prompt_template_faithfulness.replace('{model_context}', str(model_context)).replace('{statement}', str(statement)) for statement in statements]
        return (prompts, system_prompt)
    ...
    def _get_evaluation_faithfulness(self, statements: list[str], model_context: str) -> list[float | None]:
        (prompts, system_prompt) = self._get_prompts_faithfulness(statements, model_context)
        cleaned_outputs = process_prompts_to_json(prompts=prompts, system_prompts=[system_prompt], max_retry=self.max_attempts, agent=self.agent, model=self.model, json_format=StatementSupported)
        return cleaned_outputs

    def run_evaluation_pipeline(self, queries: list[str], answers: list[str], model_contexts: list[str]) -> int | None:
        ...
        for (i, statements) in enumerate(statements_list):
            if model_contexts[i] == '':
                ...
                results: list[StatementSupported] = self._get_evaluation_faithfulness(statements, model_contexts[i])
                try:
                    if any(results):
                        query_score = [result.supported for result in results].count(True) / len(results)
                        valid_queries += 1
                        mean_score = (query_score + mean_score * (valid_queries - 1)) / valid_queries
                    else:
                        return None
...
class nDCGEvaluator(Evaluator):
    ...
    def __init__(self, agent: Agent, model: str, max_attempts=5, batch_size: int=10) -> None:
        ...
        self.system_prompt = PROMPTS[self.language]['rate_chunk_relevance']['SYSTEM_PROMPT']
        self.prompt_template = PROMPTS[self.language]['rate_chunk_relevance']['QUERY_TEMPLATE']

    def _get_prompts(self, queries: list[str], answers: list[str], contexts: list[list[Chunk]]) -> list[list[tuple[str, str]]]:
        system_prompt = self.system_prompt
        ...
        for (query, answer, chunk_list) in zip(queries, answers, contexts):
            ...
            for chunk in chunk_list:
                prompt = template.replace('{query}', str(query)).replace('{answer}', str(answer)).replace('{chunk}', chunk.text)
    ...
    def rate_context(self, queries: list[str], answers: list[str], contexts: list[list[Chunk]]) -> list[list[int | None]]:
        ...
        scores_chunk = process_prompts_to_json(prompts=prompts, system_prompts=system_prompts, max_retry=self.max_attempts, agent=self.agent, model=self.model, json_format=ChunkRelevanceAnswer)
    ...
    def run_evaluation_pipeline(self, queries: list[str], answers: list[str], contexts: list[list[Chunk]]) -> float:
