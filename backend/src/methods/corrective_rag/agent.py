...
class CragAgent(NaiveRagAgent):
    ...
    def __init__(self, config_server: dict, models_infos: dict, dbs_name, data_folders_name, max_web_requests=2) -> None:
        ...
        self.websplitter = get_splitter(type_text_splitter=self.type_text_splitter, data_preprocessing=self.config_server['data_preprocessing'], agent=self.agent, embedding_model=self.embedding_model)
    ...
    def web_results_refinement(self, web_results, query, model=None, batch: bool=True):
        nb_input_tokens = 0
        nb_output_tokens = 0
        energies = [0, 0, '']
        impacts = [0, 0, '']
        ...
        for i in tqdm(range(len(web_results))):
            ...
            if batch:
                ...
                for j in range(len(chunks)):
                    prompt = self.prompts['document_relevance2']['QUERY_TEMPLATE'].format(context=chunks[j], query=query)
                    prompts.append(prompt)
                scores = self.agent.multiple_predict(prompts=prompts, system_prompt=self.system_prompt, model=self.llm_model)
                ...
                for j in range(len(chunks)):
                    prompt = self.prompts['document_relevance2']['QUERY_TEMPLATE'].format(context=chunks[j], query=query)
                    score = self.agent.predict(prompt=prompt, system_prompt=self.system_prompt, model=self.llm_model)
                    ...
                    if 'relevant' in score.lower() and 'irrelevant' not in score.lower():
                        useful_chunks.append(chunks[j])
        return {'texts': useful_chunks, 'nb_input_tokens': nb_input_tokens, 'nb_output_tokens': nb_output_tokens, 'energy': energies, 'impacts': impacts}
    ...
    def generate_answer(self, query: str, nb_chunks: str=5, batch: bool=True, options_generation=None) -> str:
        ...
        contexts = self.get_rag_context(query=query, nb_chunks=nb_chunks)[0]
        useful_contexts = []
        ambiguous_contexts = []
        if batch:
            prompts = []
            ...
            for j in range(len(contexts)):
                prompt = self.prompts['document_relevance2']['QUERY_TEMPLATE'].format(context=contexts[j].text, query=query)
                prompts.append(prompt)
        ...
        if len(useful_contexts) > 0:
            ...
            answer = agent.predict(prompt=prompt, system_prompt=system_prompt, model=self.llm_model)
            ...
            if len(web_results) > 0:
                web_results = self.web_results_refinement(web_results=web_results, query=query)
        ...
        energies[0] += answer['energy'][0]
        energies[1] += answer['energy'][1]
        return {'answer': answer['texts'], 'nb_input_tokens': nb_input_tokens, 'nb_output_tokens': nb_output_tokens, 'context': contexts, 'impacts': impacts, 'energy': energies, 'original_query': query}
