...
def process_single_doc(data_manager, processor_chunks, model: str, path_doc: str, doc_index: int, config_server: dict, agent, splitter, chunk_size: int, chunk_overlap: bool, reset_preprocess: bool, use_batch: bool) -> dict:
    ...
    data = processor_chunks.process_chunk(chunks=doc_chunks, doc_content=doc.content, model=model)
    doc_chunks = data['chunks']
    doc_indexation_tokens += np.sum(data['nb_output_tokens'])
    doc_indexation_tokens += np.sum(indexation(data_manager=data_manager, doc_chunks=doc_chunks, path_docs=path_docs))
