...
class AdvancedRag(NaiveRagAgent):
    ...
    def generate_answer(self, query: str, nb_chunks: int=7, nb_reformulation=5, options_generation=None) -> str:
        ...
        if len(chunk_list) > 0 and self.rerank:
            (rerank_chunk_list, additional_data, nb_input_tokens_rerank) = self.reranker.rerank(query=query, chunk_list=chunk_list, max_contexts=self.config_server['nb_chunks_reranker'])
        else:
        ...
        energies[0] += energy[0]
        energies[1] += energy[1]
        return {'answer': answer['texts'], 'nb_input_tokens': nb_input_tokens, 'nb_output_tokens': nb_output_tokens, 'context': rerank_chunk_list, 'impacts': impact, 'energy': energies, 'original_query': query}