...
class Weight:
    ...
    def __init__(self, relation: Relation, relations: list[Relation], entities: list[Union[MergeEntityDocument, MergeEntityOverall]]):
...
class Graph:
    ...
    def create_communities(self) -> list[list[int]]:
        partitions = la.find_partition(self.graph, la.CPMVertexPartition, resolution_parameter=0.1, weights=self.weights, n_iterations=5)
        communities = []
    ...
    def retrieve_entity_name_from_id(self, entities: list[Union[MergeEntityDocument, MergeEntityOverall]], id: int) -> str:
        for entity in entities:
            if entity.id == id:
                return entity.name
        return ''
    ...
    def convert_to_nx(self):
        G = nx.Graph()
        colors = [f'rgb({np.random.randint(0, 256)}, {np.random.randint(0, 256)}, {np.random.randint(0, 256)})' for _ in range(len(self.communities) + 1)]
    ...
    def plot_graph(self, storage_path: str, graph_name: str) -> None:
        ...
        if storage_path[-1] != '/':
            storage_path += '/'
        warnings.filterwarnings('ignore', message="Warning: When cdn_resources is 'local' jupyter notebook has issues displaying graphics on chrome/safari. Use cdn_resources='in_line' or cdn_resources='remote' if you have issues viewing graphics in a notebook.")
        net.generate_html(name=f'{storage_path}{graph_name}.html', local=True, notebook=False)
