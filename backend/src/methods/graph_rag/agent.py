...
class GraphRagAgent(BaseRAGAgent):
    ...
    def __init__(self, config_server: dict, models_infos: dict, dbs_name: list[str], data_folders_name: list[str]) -> None:
        super().__init__(config_server=config_server, models_infos=models_infos, dbs_name=dbs_name, data_folders_name=data_folders_name, rag_name='graph')
    ...
    def indexation_phase(self, reset_index: bool=False, overlap: bool=True, reset_preprocess=False) -> None:
        ...
        if reset_index:
            for db_name in self.dbs_name:
                self.data_manager.delete_collection(vb_name=db_name, name='graph_rag_local')
    ...
    def generate_answer(self, query: str, method: str='global', nb_chunks: str=2, options_generation=None) -> dict:
        ...
        (context, chunks, tokens_counter) = self.get_rag_context(query=query, method=method, nb_chunks=nb_chunks)
