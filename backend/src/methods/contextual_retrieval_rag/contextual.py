...
def run_contextual(agent, model, doc_chunks, doc_content, language='EN', first_process=True):
    chunk_with_context = []
    user_prompts = []
    system_prompts = []
    for i in range(len(doc_chunks)):
        context_chunk = get_neighbors_chunks(doc_chunks=doc_chunks, index=i, max_context_len=8192)
        prompt = prompts[language]['generate_context']['QUERY_TEMPLATE'].replace('{CHUNK_CONTENT}', doc_chunks[i].text).replace('{WHOLE_DOCUMENT}', context_chunk)
    ...
    for i in range_chunks:
        results = agent.multiple_predict(prompts=user_prompts[i:i + taille_batch], system_prompt=system_prompt, model=model)
        if contexts is None:
            contexts = results
        else:
    ...
    return {'texts': chunk_with_context, 'nb_output_tokens': nb_output_tokens, 'nb_input_tokens': nb_input_tokens, 'impacts': contexts['impacts'], 'energy': contexts['energy']}