...
class MergerRagAgent(NaiveRagAgent):
    ...
    def merge_answers(self, answers: list[str], query: str, agent, options_generation=None) -> str:
        ...
        system_prompt = "Tu es un assistant expert en synthèse d'information. Tu dois combiner plusieurs réponses à une même question pour produire une réponse unique et complète"
        user_prompt = f"Voici la question :\n{query}\n\nTu as reçu {len(answers)} réponses de différents systèmes RAG. Voici ces réponses :\n\n{formatted_answers}\n\nTa tâche est de produire une réponse synthétique, précise et complète. Si plusieurs réponses disent la même chose, ne le répète pas inutilement. S'il y a des divergences, essaie de les expliquer ou de les concilier."
        return agent.predict(prompt=user_prompt, system_prompt=system_prompt, options_generation=options_generation, model=self.llm_model)

    def generate_answer(self, query: str, nb_chunks: int=5, options_generation=None) -> str:
        self.list_answers = {}
        full_response = {'answer': '', 'nb_input_tokens': 0, 'nb_output_tokens': 0, 'context': [], 'impacts': [0, 0, ''], 'energy': [0, 0, '']}
        for rag_name in self.rag_list:
            response = self.rag_agents[rag_name].generate_answer(query=query, nb_chunks=nb_chunks, options_generation=options_generation)
        ...
        return {'answer': full_response['answer'], 'nb_input_tokens': full_response['nb_input_tokens'], 'nb_output_tokens': full_response['nb_output_tokens'], 'context': full_response['context'], 'impacts': full_response['impacts'], 'energy': full_response['energy'], 'original_query': query}