...
def generates_questions(chunks: list[str], model: str, prompts_template, agent, temperature=1, text_to_show='Generate questions'):
    prompts = []
    for chunk in chunks:
    ...
    k = 0
    for i in data_range:
        results = agent.multiple_predict(prompts=prompts[i:i + taille_batch], model=model, system_prompt=system_prompt)
        k += 1
...
def indexation(data_manager, doc_chunks, path_docs, model, agent, prompts_template):
    ...
    list_questions = generates_questions(chunks=elements, agent=agent, prompts_template=prompts_template, model=model, text_to_show='Generate questions')
    ...
    for i in range(0, len(final_chunks), taille_batch):
        embedding_tokens += np.sum(data_manager.add_str_batch_elements(display_message=False, chunks=final_chunks[i:i + taille_batch], path_docs=final_path_docs[i:i + taille_batch]))
    return (embedding_tokens, input_tokens, output_tokens)

def process_single_doc(data_manager, path_doc: str, doc_index: int, config_server: dict, agent, model, splitter, chunk_size: int, chunk_overlap: bool, reset_preprocess: bool, prompts_template) -> dict:
    (embedding_tokens, input_tokens, output_tokens) = (0, 0, 0)
    doc = DocumentText(path=path_doc, doc_index=doc_index, config_server=config_server, splitter=splitter, agent=agent, reset_preprocess=reset_preprocess)
    ...
    (embedding_t, input_t, output_t) = indexation(data_manager=data_manager, agent=agent, prompts_template=prompts_template, doc_chunks=doc_chunks, path_docs=path_docs, model=model)
    embedding_tokens += embedding_t
    input_tokens += input_t
    output_tokens += output_t
    new_doc = Document(name=str(Path(path_doc).name), path=str(Path(path_doc)), embedding_tokens=int(embedding_tokens), input_tokens=int(input_tokens), output_tokens=int(output_tokens))
    return new_doc

class QbRagIndexation:

    def __init__(self, data_manager, agent, embedding_model, llm_model, data_preprocessing, language: str='EN', type_text_splitter='TextSplitter'):
    ...
    def run_pipeline(self, config_server, chunk_size, chunk_overlap: bool=True, reset_preprocess: bool=False, max_workers: int=10):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                self.data_manager.add_instance(instance=new_doc, path=str(Path(path_doc).parent))
        progress_bar.clear()