...
def indexation(data_manager, doc_chunks, path_docs):
    ...
    for i in range_chunks:
        tokens += np.sum(data_manager.add_str_batch_elements(chunks=doc_chunks[i:i + taille_batch], path_docs=path_docs[i:i + taille_batch], display_message=False))
    return tokens

def process_single_doc(data_manager, path_doc: str, doc_index: int, config_server: dict, agent, splitter, chunk_size: int, chunk_overlap: bool, reset_preprocess: bool) -> dict:
    ...
    doc_indexation_tokens = indexation(data_manager=data_manager, doc_chunks=doc_chunks, path_docs=path_docs)
    return {'name': str(Path(path_doc).name), 'path': str(path_doc), 'embedding_tokens': int(doc_indexation_tokens), 'parent_path': str(Path(path_doc).parent)}

class NaiveRagIndexation:

    def __init__(self, data_manager, agent, embedding_model, data_preprocessing: str, type_text_splitter='TextSplitter') -> None:
