...
database_name = left.selectbox(label='Choose database', options=st.session_state['all_databases'], label_visibility='collapsed', key='db_name', on_change=change_metadatas_keys)
if 'metadata' not in st.session_state:
    st.session_state.metadata = load_metadatas_keys(database_name)
st.markdown('## Add new metadata key')
(left, right) = st.columns([6, 1], vertical_alignment='bottom')
st.session_state['new_metadata'] = left.text_input(label='**New metadata**', placeholder='Metadata key : alphanumeric characters, underscores and hyphens only', label_visibility='collapsed')
if right.button(label='Add metadata key', type='primary', use_container_width=True):
    ...
    save_new_metadata_key(database_name=database_name, new_key=st.session_state['new_metadata'])
    st.session_state['metadata'] = load_metadatas_keys(database_name)
...
if st.session_state['metadata'] != []:
    ...
    key_to_change = left.selectbox(label='Choose the metadata to change', options=keys_available)
    ...
    if middle.button(label='Global modification', type='primary', use_container_width=True):
        ...
        st.session_state.individual_ui = False
        st.rerun()
    if right_col.button(label='Individual modification', type='primary', use_container_width=True):
...
if st.session_state.global_ui:
    new_input = st.text_input(label='*The metadata to add', placeholder=f'**{key_to_change}**', label_visibility='collapsed')
    ...
    if right.button(label='Save', type='primary', use_container_width=True):
        save_global_modification(database_name, key_to_change, new_input, selected_docs)
        st.session_state.global_mod_active = False
...
if st.session_state['metadata'] != []:
    ...
    if st.session_state.delete_ui:
        ...
        keys_available = load_metadatas_keys(database_name)
        key_to_delete = left.selectbox(label='Choose the metadata to delete', options=keys_available)
        if middle.button(label='Delete', type='primary', use_container_width=True):
            delete_metadata(database_name, key_to_delete)
            st.session_state.delete_ui = False
            st.session_state.button = True
            st.rerun()
        if right.button(label='Cancel', type='primary', use_container_width=True):
            st.session_state.delete_ui = False
            st.session_state.button = True
            st.rerun()