...
pg = st.navigation(([chat, config, benchmark, 
                     rag_maker, databases, documentation]))
...
if "config_server" not in st.session_state:
    with open("streamlit_/utils/base_config_server.json", "r") as file:
        ...
        if base_config_server["params_vectorbase"]['backend']=="elasticsearch":
            base_config_server["params_vectorbase"]["url"] = os.getenv("ES_LOCAL_URL")+":"+os.getenv("ES_LOCAL_PORT")
            base_config_server["params_vectorbase"]["auth"] = ["elastic", 
    ...
    for file in glob.glob("data/custom_rags/ollama_openai_mistral/*.json"):
        ...
        if config["params_vectorbase"]['backend']=="elasticsearch":
            config["params_vectorbase"]["url"] = os.getenv("ES_LOCAL_URL")+":"+os.getenv("ES_LOCAL_PORT")
            config["params_vectorbase"]["auth"] = ["elastic", 
                                                        os.getenv("ES_LOCAL_PASSWORD")]
...
if "api_key" not in st.session_state:
    st.session_state["api_key"] = st.session_state["config_server"]["params_host_llm"]["api_key"]
...
if "custom_rags" not in st.session_state:
    ...
    custom_rags = [custom_rag[:-5] for custom_rag in custom_rags if custom_rag != ".gitkeep"]
